...
class ViT(nn.Module):
    ...
    def forward(self, feature_tensor):
        ...
        # 直接返回全部 patch token 序列，不池化
        return feature_tensor_for_vit
class ArcMarginProduct(nn.Module):
    """
    ArcFace margin classification head.
    During training: requires label and computes angular margin logits.
    During inference: returns scaled cosine similarity without margin.
    """
...
class CTSNet_firstorder(nn.Module):
    """
    Original CompNet: CB1 // CB2 // CB3 + FC + Dropout + ArcMargin.
    Reference: https://ieeexplore.ieee.org/document/9512475
    Multi-branch competitive feature extractor with ViT branch.
    """
    def __init__(self, num_classes):
        ...
        self.cb2 = CompetitiveBlock(channel_in=1, n_competitor=36, ksize=17, stride=3, padding=0, init_ratio=0.5)
        self.cb3 = CompetitiveBlock(channel_in=1, n_competitor=18, ksize=7, stride=3, padding=0, init_ratio=0.25)
    ...
    def _forward_shared(self, x):
        """Shared feature extraction branch, no ArcMargin, returns raw concatenated feature."""
        x1_1= self.cb1(x)
        x2 = self.cb2(x)
        x3 = self.cb3(x)
        ...
        # Feed CB1 feature map into ViT, use mean pooling
        x1_1 = self.vit1(x1_1)
        ...
        # 三路 x1, x2, x3 形状均 [B,1024]
        stacked = torch.stack([x1, x2, x3], dim=1)  # [B,3,1024]
        # 简单版本：先全局池化算分支重要性，再生成权重
        score = stacked.mean(dim=-1)  # [B,3]
        attn = F.softmax(score, dim=-1)  # [B,3] 样本自适应尺度竞争权重
    ...
    def forward(self, x, labels=None):
        """Training forward: return ArcFace logits and raw feature vector."""
    ...
    def get_embedding(self, x):
        """Evaluation interface: return L2-normalized embedding for matching."""
        with torch.no_grad():
            feat = self._forward_shared(x)
...
if __name__ == "__main__":
    ...
    # Dummy input: batch=4, single-channel grayscale palmprint 128×128
    batch_size = 4
    ...
    # 2. Evaluation interface get_embedding (use this in validation/test)
    ...
    # 3. Single image test (simulate single sample from DataLoader)
    ...
    # 4. Train mode forward (dropout enabled)
    model.train()
